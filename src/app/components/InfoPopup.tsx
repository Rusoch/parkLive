...
const InfoPopup: React.FC<TProps> = ({ isOpen, onClose, placeData }) => {
    ...
    return (
        <div className="bg-white dark:bg-[#0D0D0D] rounded-lg shadow-lg flex items-center flex-col px-[14px] text-[#15593A] dark:text-[#15593A] text-[20px]">
            ...
            <ul className="flex flex-col w-full py-4 items-start gap-1">
            ...
            <div className="flex w-full justify-around mt-8 mb-9">
                <Button
                    ...
                    label={t("directions")}
                    className="rounded-[14px] w-[44%] py-2 border bg-[#15593A] text-white dark:text-[#0D0D0D] dark:bg-[#1A6E48]"
                />
                <Button
                    onClick={() => console.log("შენახვა")}
                    label={t("save")}
                    className="rounded-[14px] w-[44%] py-2 border border-[#15593A] text-[#15593A] dark:text-[#1A6E48] dark:border-[#1A6E48]"
                />
            </div>
        </div>
    );
